...
public class SingupServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (password == null || username == null) {
            request.setAttribute("error", "Sorry username and password is invalid");
            request.getRequestDispatcher("singup.jsp").forward(request, response);
        } else {
            User ur = dao.getUser(username);
            if (ur != null) {
                request.setAttribute("error", "The username is already exist");
                request.getRequestDispatcher("singup.jsp").forward(request, response);
            } else {
                User a = new User(1, username, password, nickname, email, phone, address, 2);
                if (dao.insert(a) == 1) {
                    request.getRequestDispatcher("singupsucess.jsp").forward(request, response);
                } else {
                    response.sendRedirect("error-pages/403.jsp");
                }
            }
        }
